//
//  eth_hwif  -  Ethernet Hardware Interface
//  
//  h/w driver routines originally written by TIdude
//
//  the ethernet module is a single switch that has 3 ports
//  port 0 is for host access via a separate dedicated DMA module
//  port 1 is connected to the external connector
//  port 2 is not available, as the NSA uses it to snoop
//  individual port controllers are called "slivers"
//  the switch serves as a junction box for connecting ports
//
...
// n! the following additional defs & functions are inserted
...
#define MAC_ID0_LO                         0x630   // n!
//
// MACAddrGet for the BBB
//
// there is a MAC for each of the 2 external ethernet ports (aka slivers) spruh73n 14.2
...
//
//  Reset  eth/CPPI DMA Controller
//
//  baseAddr      Base address of CPPI DMA Module registers
//
//  return        None
//
//  NB  the ethernet module has its own DMA
//
void ethDMA_reset(unsigned int baseAddr){
    unsigned int cnt;
    ...
    while(HWREG(baseAddr + CPSW_CPDMA_CPDMA_SOFT_RESET)
          & CPSW_CPDMA_CPDMA_SOFT_RESET_SOFT_RESET);
    // Initialize all header descriptor pointer registers
 ...
 *
 **/
void ethDMA_TxIntEnab(unsigned int baseAddr, unsigned int channel)
{
 ...
 *
 **/
void ethDMA_RxIntEnab(unsigned int baseAddr, unsigned int channel)
{
 ...
 *
 **/
void ethDMA_TxEnab(unsigned int baseAddr)
{
 ...
 *
 **/
void ethDMA_RxEnab(unsigned int baseAddr)
{
 ...
 *
 **/
void ethDMA_TxHdrDescPWr(unsigned int baseAddr, unsigned int descHdr,
 ...
 **/
void ethDMA_RxHdrDescPWr(unsigned int baseAddr, unsigned int descHdr,
                                unsigned int channel)
{
 ...
 * 
 **/
void ethDMA_EOIVecWr(unsigned int baseAddr, unsigned int eoiFlag)
{
 ...
 *
 **/
void ethDMA_TxCPWr(unsigned int baseAddr, unsigned int channel, 
                        unsigned int comPtr)
{
 ...
 *
 **/
void ethDMA_RxCPWr(unsigned int baseAddr, unsigned int channel, 
                        unsigned int comPtr)
{
